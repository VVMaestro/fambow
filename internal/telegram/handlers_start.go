...
func registerCoreHandlers(b *bot.Bot, logger *slog.Logger, loveNotes LoveNoteProvider, memories MemoryProvider, reminders ReminderProvider, loveSchedules LoveNoteScheduleProvider, celebrations CelebrationProvider, products ProductProvider, users UserProvider, adminTelegramUserID int64, memoryWizard *memoryWizardState, reminderWizard *reminderWizardState, eventWizard *eventWizardState, loveScheduleWizard *loveScheduleWizardState) {
	...
	b.RegisterHandler(bot.HandlerTypeMessageText, "Events", bot.MatchTypePrefix, guard(eventsHandler(logger, celebrations)))
	...
	b.RegisterHandler(bot.HandlerTypeMessageText, "/list_reminders", bot.MatchTypePrefix, listAllRemindersHandler(logger, reminders, adminTelegramUserID))
	b.RegisterHandler(bot.HandlerTypeMessageText, "/remove_reminder", bot.MatchTypePrefix, removeReminderHandler(logger, reminders, adminTelegramUserID))
	...
	b.RegisterHandler(bot.HandlerTypeMessageText, "/list_love_notes", bot.MatchTypeExact, listLoveNotesHandler(logger, loveNotes, adminTelegramUserID))
	b.RegisterHandler(bot.HandlerTypeMessageText, "/delete_love_notes", bot.MatchTypePrefix, deleteLoveNotesHandler(logger, loveNotes, adminTelegramUserID))
